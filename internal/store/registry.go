...
// LOCALLY enrolled identity — the authoritative auth source. The _EnrolledIdentity
// entity record written in the same batch is the same fact as visible,
// replicating state; a replicated _EnrolledIdentity from a child updates KV/history
...
// RegistryPut writes the registry entry and appends its _EnrolledIdentity entity
...
// (empty-payload _EnrolledIdentity tombstone) and deletes the identity's KV
