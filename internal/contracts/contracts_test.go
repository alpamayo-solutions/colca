...
func TestLoadRealGeneratedBundle(t *testing.T) {
	// Generated by scripts/dev.py bundle into test-results; regenerate here
	// via the checked-in generator if absent — keep the test hermetic by
	// looking for any pre-generated file first.
	candidates, _ := filepath.Glob("../../../test-results/bundle/*.json")
	if len(candidates) == 0 {
		t.Skip("no generated bundle present (run `uv run scripts/dev.py bundle`) — the level-2 parity gate covers generation")
	}
	tbl, err := Load(candidates[0], "")
	if err != nil {
		t.Fatalf("real generated bundle failed to load: %v", err)
	}
	raw, _ := os.ReadFile(candidates[0])
	var f struct {
