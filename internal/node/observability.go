...
func nodeHealthMetrics() []map[string]any {
	return []map[string]any{
		{"key": "health", "name": "Service health", "metric": "hub:services_healthy_pct", "description": "Share of services on this node reporting healthy.", "query": `max(hub:services_healthy_pct{{node_id="{node_id}"}}) * 100`, "visualization": "gauge", "unit": "%", "precision": 1, "min_value": 0, "max_value": 100, "thresholds": map[string]any{"warning": 90, "critical": 50}},
		{"key": "cpu", "name": "CPU", "metric": "hub:cpu_idle_pct", "description": "Node CPU utilization.", "query": `max((1 - hub:cpu_idle_pct{{node_id="{node_id}"}}) * 100)`, "visualization": "timeline", "unit": "%", "precision": 1, "min_value": 0, "max_value": 100, "thresholds": map[string]any{"warning": 70, "critical": 90}},
		{"key": "disk", "name": "Disk", "metric": "hub:disk_used_pct", "description": "Node disk utilization.", "query": `max(hub:disk_used_pct{{node_id="{node_id}"}}) * 100`, "visualization": "timeline", "unit": "%", "precision": 1, "min_value": 0, "max_value": 100, "thresholds": map[string]any{"warning": 70, "critical": 90}},
		{"key": "replication_lag", "name": "Replication lag", "metric": "colca_cursor_lag_records", "description": "Records waiting behind this node's replication cursors.", "query": `max(colca_cursor_lag_records{{node_id="{node_id}"}})`, "visualization": "timeline", "unit": "records", "precision": 0, "min_value": 0, "thresholds": map[string]any{}},
	}
}
